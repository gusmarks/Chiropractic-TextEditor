...
public:
	...
	/// constructor
	/// </summary>
	/// <param name="parent"> parrent is a wx window</param>
	/// <param name="id"> a uniqe id</param>
	ArchiveManager(wxWindow* parent, wxWindowID id) :wxDialog(parent, id, "Archive Manager", wxDefaultPosition, wxSize(500, 500), wxDEFAULT_DIALOG_STYLE, "")
	{
		try {
			...
			popUpHandeler->errorMessage("Error making the Archive Manager.");
		}
	...
	void Billed_Unbilled(wxCommandEvent& event) {
		try {
			...
			popUpHandeler->errorMessage("Error changing billed status.");
		}

	}
	/// <summary>
	/// swaps the "payed" bit on the file name
	/// </summary>
	/// <param name="event"></param>
	void payed_Unpayed(wxCommandEvent& event) {
		try {
			...
			popUpHandeler->errorMessage("Error changing payed status.");
		}
	}
	/// <summary>
	/// bild archive files, and sorts patient files that have been both billed and payed into the archives
	/// </summary>
	/// <param name="event"></param>
	void archiveFilesFunction(wxCommandEvent& event) {
		try {
			...
			for (size_t n = 0; n < size; n++) {
				indicies.push_back(PatientIndicies[n]);
			}
			size = indicies.size();
			for (size_t i = 0; i < size; i++) {
				tempPatientFileName = patientsToArchive[PatientIndicies[i]];
				...
				if (checkBilled == '1' && checkPayed == '1') {
					...
					popUpHandeler->errorMessage("At least one of the selected patients Either have not been billed or payed.");
				}
			}
			int j = 0;
			int indiciesCount = indicies.size()-1;
			for ( j= indiciesCount; j >= 0; j--) {
				patientsViewBox->Delete(indicies[j]);
			}
		}
		catch (...) {
			popUpHandeler->errorMessage("Error archiving files");
		}
	}
	...
	wxArrayString gatherPatients() {
		try {
			...
			popUpHandeler->errorMessage("Error gathering Patient files");
		}
	}
	std::vector<std::string> gatherPatientFolders() {
		try{
			...
			if (!results.empty()) {
				...
				popUpHandeler->errorMessage("Error gathering Patient Folders");
		}
	}
	void setPatientsToArchive(wxArrayString arr) {
		try {
			...
			popUpHandeler->errorMessage("Error Setting Archive");
		}
	}
